...
High-level view of article counts, labels, and regional distribution.
Label trends shown in separate subplots with 12-month rolling mean and full-span mean.
"""
...
from utils.filters import render_source_filter, render_date_filter, apply_filters, render_summary_metrics
...
sources = render_source_filter(df, "p1")
date_range = render_date_filter(df, "p1")

# Apply filters
filtered_df = apply_filters(df, sources=sources, date_range=date_range)
...
if filtered_df.empty:
    ...
    # 1. Total Article Counts Over Time
    st.subheader("1. Total Article Counts Over Time")

    ts_total = filtered_df.groupby("yearmon").size().reset_index(name="count")
    ts_total["yearmon_date"] = pd.to_datetime(ts_total["yearmon"])

    chart_total = alt.Chart(ts_total).mark_line(point=True, strokeWidth=2).encode(
    ...
    top_n_adm1 = st.slider("Top N ADM1 Regions", 5, 20, 10, key="p1_topn_adm1")
    ...
    top_n_adm2 = st.slider("Top N ADM2 Counties", 10, 50, 20, key="p1_topn_adm2")
